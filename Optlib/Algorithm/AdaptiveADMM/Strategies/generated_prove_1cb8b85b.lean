...
def scale_seq (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) (n : ℕ) : ℝ :=
  if r_ratio r_norm_seq s_norm_seq eps n > mu * (1 + 0.1 * Real.log (1 + (n : ℝ))) then
    min 2 (max 1 (r_ratio r_norm_seq s_norm_seq eps n / (2 * mu * (1 + 0.1 * Real.log (1 + (n : ℝ))))))
  else if s_ratio r_norm_seq s_norm_seq eps n > mu * (1 + 0.1 * Real.log (1 + (n : ℝ))) then
    min 2 (max 1 (s_ratio r_norm_seq s_norm_seq eps n / (2 * mu * (1 + 0.1 * Real.log (1 + (n : ℝ))))))
  else
    1

def dir_seq (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) (n : ℕ) : ℤ :=
  if r_ratio r_norm_seq s_norm_seq eps n > mu * (1 + 0.1 * Real.log (1 + (n : ℝ))) then 1
  else if s_ratio r_norm_seq s_norm_seq eps n > mu * (1 + 0.1 * Real.log (1 + (n : ℝ))) then -1 else 0
...
lemma h_dir (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) :
  ...
  intro n
  by_cases h1 : r_ratio r_norm_seq s_norm_seq eps n > mu * (1 + 0.1 * Real.log (1 + (n : ℝ)))
  · simp [dir_seq, h1]
  · by_cases h2 : s_ratio r_norm_seq s_norm_seq eps n > mu * (1 + 0.1 * Real.log (1 + (n : ℝ)))
    · simp [dir_seq, h1, h2]
    · simp [dir_seq, h1, h2]

def update_fun (tau : ℕ → ℝ) (dir : ℕ → ℤ) (scale : ℕ → ℝ) (n : ℕ) (rho : ℝ) : ℝ :=
  if dir n = (-1 : ℤ) then
    rho / (1 + tau n * scale n)
...
lemma h_update_equiv (tau : ℕ → ℝ) (dir : ℕ → ℤ) (scale : ℕ → ℝ)
    (h_dir : ∀ n, dir n = 1 ∨ dir n = 0 ∨ dir n = -1) :
    ∀ n rho, 0 < rho →
      update_fun tau dir scale n rho = rho * (1 + tau n * scale n) ∨
      update_fun tau dir scale n rho = rho / (1 + tau n * scale n) ∨
      update_fun tau dir scale n rho = rho := by
  intro n rho hρ_pos
...
theorem auto_converges
    ...
    (h_rho : ∀ n, admm.ρₙ (n+1) = update_fun (tau_seq c p) (dir_seq mu eps r_norm_seq s_norm_seq) (scale_seq mu eps r_norm_seq s_norm_seq) n (admm.ρₙ n))
  ...
  let scale := scale_seq mu eps r_norm_seq s_norm_seq
  ...
  let s : AdaptableStrategy (admm := admm) (admm_kkt := admm_kkt) :=
    { tau_seq := tau
      ...
      update_fun := update_fun tau dir scale
      h_update_equiv := h_update_equiv tau dir scale h_dir' }
