...
def mu_seq (mu : ℝ) (tau : ℕ → ℝ) (n : ℕ) : ℝ := mu * (1 + tau n)
...
def dir_seq (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) (tau : ℕ → ℝ) (n : ℕ) : ℤ :=
  if R_ratio r_norm_seq s_norm_seq eps n > mu_seq mu tau n then 1
  else if S_ratio r_norm_seq s_norm_seq eps n > mu_seq mu tau n then -1 else 0
...
lemma h_dir (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) (tau : ℕ → ℝ) :
  ...
  intro n
  by_cases h1 : R_ratio r_norm_seq s_norm_seq eps n > mu_seq mu tau n
  · simp [dir_seq, h1]
  · by_cases h2 : S_ratio r_norm_seq s_norm_seq eps n > mu_seq mu tau n
    · simp [dir_seq, h1, h2]
    · simp [dir_seq, h1, h2]
