...
def dir_seq (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) (n : ℕ) : ℤ :=
  if r_tilde r_norm_seq s_norm_seq eps n > mu then 1
  else if s_tilde r_norm_seq s_norm_seq eps n > mu then -1 else 0
...
lemma h_dir (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) :
  ...
  intro n
  by_cases h1 : r_tilde r_norm_seq s_norm_seq eps n > mu
  · simp [dir_seq, h1]
  · by_cases h2 : s_tilde r_norm_seq s_norm_seq eps n > mu
    · simp [dir_seq, h1, h2]
    · simp [dir_seq, h1, h2]
